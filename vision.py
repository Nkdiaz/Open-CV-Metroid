...
class Vision:
    ...
    # constructor
    def __init__(self, door_img_path, method=cv.TM_CCOEFF_NORMED):
        if door_img_path:
            # load the image we're trying to match
            self.door_img = cv.imread(door_img_path, cv.IMREAD_UNCHANGED)
            # Save the dimensions of the door image
            self.door_w = self.door_img.shape[1]
            self.door_h = self.door_img.shape[0]

        # There are 6 methods to choose from:
        # TM_CCOEFF, TM_CCOEFF_NORMED, TM_CCORR, TM_CCORR_NORMED, TM_SQDIFF, TM_SQDIFF_NORMED
        self.method = method

    def find(self, background_img, threshold=0.5, max_results=10):
        # run the OpenCV algorithm
        result = cv.matchTemplate(background_img, self.door_img, self.method)
        ...
        for loc in locations:
            rect = [int(loc[0]), int(loc[1]), self.door_w, self.door_h]
            # Add every box to the list twice in order to retain single (non-overlapping) boxes. Its a weird numpy thing
    ...
    def match_keypoints(self, original_image, patch_size=32):
        ...
        keypoints_needle, descriptors_needle = orb.detectAndCompute(self.door_img, None)
